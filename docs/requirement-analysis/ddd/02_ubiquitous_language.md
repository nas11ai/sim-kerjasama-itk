...
| **FormPhaseDetail**        | Step spesifik dalam FormPhase — kombinasi FormAccessControl + SubmissionDate + PhaseType + order + `needs_review`. Punya hard deadline via `submission_date_id` (NOT NULL). `needs_review = true` menjadi gate workflow: step selanjutnya tidak bisa diakses sebelum reviewer approve. |
| **FormAccessControl**      | Aturan akses sebuah Form: kombinasi `permission` (string) + `organization_id`. Permission di-check via Spatie — cover dua jalur: inherited dari role maupun direct assign ke user.                                                                                                     |
...
| **SubmissionDate**         | Deadline yang terhubung ke satu atau lebih FormPhaseDetail. Punya `label` deskriptif (e.g., "Batas Submit Pengajuan", "Batas Revisi") dan `datetime`. Setelah `datetime` terlewat, akses ke FormPhaseDetail yang terhubung ditutup secara hard.                                        |
...
| **FormSubmissionOverride** | Record yang dibuat Operator untuk membuka kembali akses ke satu FormPhaseDetail untuk satu FormSubmission spesifik setelah deadline lewat. Punya `reason` (wajib), `expires_at` (opsional), dan `is_active`. Tercatat permanen untuk audit trail.                                      |
| **Hard Deadline**          | Batas waktu yang bersifat absolut — lewat deadline, akses ditutup tanpa peringatan yang bisa di-bypass. Satu-satunya pengecualian adalah FormSubmissionOverride dari Operator. Berbeda dengan soft deadline yang masih memperbolehkan submit dengan status "terlambat".                |
