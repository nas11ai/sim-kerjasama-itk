...
const isAssignedReviewer = computed(
    () => props.userRole === "reviewer" && props.canReview,
);
...
const isThreadExpanded = (threadId: number) =>
    expandedThreads.value.has(threadId);
...
const getThreadComments = (reviewSummaryId: number) => {
    return props.reviewComments.filter(
        (c) => c.review_summary_id === reviewSummaryId,
    );
};
...
const canActuallyCreateThread = computed(() => {
    ...
    const requiredForms = reviewerAssignments.value.filter(
        (a) => a.is_required,
    );
    ...
    return (
        ...
        completedRequired.length === requiredForms.length
    );
});
...
const getAuthorDisplay = (comment: ReviewComment) => {
    if (comment.reviewer?.user) {
        ...
        return {
            ...
            type:
                comment.user.id === currentUser.value?.id ? "You" : "Submitter",
            icon: User,
...
const downloadAttachment = (filePath: string) => {
    window.open(
        `/review-attachments/download?path=${encodeURIComponent(filePath)}`,
        "_blank",
    );
};
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-muted-foreground text-sm mt-1">
                            Lengkapi form evaluasi yang wajib sebelum membuat
                            thread review
                        </p>
                    </div>
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    <h4 class="font-medium">
                                        {{
                                            assignment.review_evaluation_form
                                                .title
                                        }}
                                    </h4>
                                    <Badge
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <Button
                                    ...
                                    @click="
                                        router.visit(
                                            route(
                                                'reviewer.evaluation-form.submitted',
                                                { assignment: assignment.id },
                                            ),
                                        )
                                    "
                ...
                >
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex items-center gap-3 flex-1">
                                <Avatar>
                                    <AvatarFallback>
                                        {{
                                            reviewSummary.reviewer?.user.name.charAt(
                                                0,
                                            ) || "?"
                                        }}
                                    </AvatarFallback>
                            ...
                            <Button
                                ...
                                size="sm"
                                @click.stop="
                                    openUpdateReviewStatusDialog(reviewSummary)
                                "
                                class="ml-2"
                            >
                                <Edit class="h-4 w-4" />
                    ...
                    <CollapsibleContent>
                        <CardContent class="space-y-4 pt-0">
                            ...
                            <!-- Attachments -->
                            <div
                                v-if="reviewSummary.attachments.length > 0"
                                class="space-y-2"
                            >
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <Badge variant="outline" class="text-xs">
                                        {{
                                            getThreadComments(reviewSummary.id)
                                                .length
                                        }}
                                        Komentar
                                    </Badge>
                                ...
                                >
                                    <div
                                        v-for="comment in getThreadComments(
                                            reviewSummary.id,
                                        )"
                                        :key="comment.id"
                                        class="space-y-3"
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <p
                                            class="text-xs text-muted-foreground"
                                        >
                                            Komentar dinonaktifkan untuk thread
                                            dengan status
                                            {{ reviewSummary.status }}
                                        </p>
                                    </div>
            ...
            <DialogContent class="max-w-md">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-end gap-2">
                        <Button
                            variant="outline"
                            @click="showUpdateReviewStatusDialog = false"
                            >Batal</Button
                        >
                        <Button
            ...
            <DialogContent class="max-w-md">
                ...
                <div class="space-y-4">
                    ...
                    <Alert>
                        ...
                        <AlertDescription>
                            Ini akan memperbarui status pengajuan untuk semua
                            reviewer dan pengaju.
                        </AlertDescription>
                    </Alert>
                    <div class="flex justify-end gap-2">
                        <Button
                            variant="outline"
                            @click="showUpdateSubmissionStatusDialog = false"
                            >Batal</Button
                        >
                        <Button
