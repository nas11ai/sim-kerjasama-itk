...
const periodStartDate = computed(() => {
    if (
        !props.submissionPeriod.submission_dates ||
        props.submissionPeriod.submission_dates.length === 0
    ) {
        return 'N/A'
    }
    const earliestDate = props.submissionPeriod.submission_dates.reduce((earliest, current) => {
...
const periodEndDate = computed(() => {
    ...
    const latestDate = props.submissionPeriod.submission_dates.reduce((latest, current) => {
        const currentDate = new Date(current.datetime)
...
<template>
    <Head :title="`${submissionPeriod.name} - Pengajuan`" />
                ...
                <CardContent>
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            >
                            <p class="font-medium">
                                {{ submissionPeriod.description || 'Tidak ada deskripsi tersedia' }}
                            </p>
                        </div>
                    </div>
                    <div v-if="submissionPeriod.submission_dates.length > 0" class="mt-4">
                        <Label class="text-sm font-medium text-muted-foreground"
                            >Tanggal Penting</Label
                        >
            ...
            <Card>
                ...
                <CardContent>
                    <div class="grid gap-4 md:grid-cols-4">
                        <!-- Form Phase Select -->
                        <div>
                            <Label for="form_phase">Tahap Formulir</Label>
                        ...
                        <div>
                            ...
                            <Select v-model="statusFilter">
                                ...
                                <SelectContent>
                                    <SelectItem
                                        v-for="option in submissionStatuses"
                                        :key="option.value"
                                        :value="option.value"
                                    >
                                        {{ option.label }}
                ...
                <CardContent>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                <TableRow v-if="submissions.data.length === 0">
                                    ...
                                    >
                                        Tidak ada pengajuan yang ditemukan untuk kriteria yang
                                        dipilih.
                                    </TableCell>
                                </TableRow>
                                <TableRow
                                ...
                                >
                                    ...
                                    <TableCell>
                                        <Badge
                                            :variant="
                                                getSubmissionStatusBadge(submission.status).variant
                                            "
                                            class="text-sm"
                                        >
                                            <component
                                                :is="
                                                    getSubmissionStatusBadge(submission.status).icon
                                                "
                                                class="h-3 w-3 mr-1"
                                            />
                    ...
                    >
                        <div class="text-sm text-muted-foreground">
                            Menampilkan {{ submissions.from }} hingga {{ submissions.to }} dari
                            {{ submissions.total }} hasil
                        </div>
