...
const submitForm = () => {
    ...
    if (hasFileUploads) {
        ...
        responses.forEach((response, index) => {
            formDataPayload.append(`responses[${index}][form_field_id]`, String(response.form_field_id));
        ...
        formData.transform(() => formDataPayload).post(route("user.biodata.submit"), {
            ...
            onError: (errors) => {
                ...
                showCustomAlert(
                    'error',
                    'Gagal Mengirim',
                    typeof firstError === 'string' ? firstError : 'Terjadi kesalahan saat mengirim biodata.'
                );
            },
            onFinish: () => {
