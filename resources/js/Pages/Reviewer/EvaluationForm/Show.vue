...
import { ArrowLeft, Save, Send, FileText, User, AlertTriangle, Calendar } from 'lucide-vue-next'
...
const hasChanges = computed(() => {
    return (
        JSON.stringify(form.responses) !== JSON.stringify(props.existingResponses) ||
        form.final_notes !== ''
    )
})
...
const requiredFieldsCompleted = computed(() => {
    const requiredFields = props.assignment.review_evaluation_form.review_form_fields.filter(
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                <Button
                    variant="ghost"
                    size="sm"
                    @click="$inertia.visit(route('reviewer.assignments.index'))"
                >
        ...
        <div class="max-w-4xl mx-auto space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <div class="space-y-4">
                            ...
                            >
                                ...
                                <span
                                    >Terakhir disimpan:
                                    {{ formatDate(lastSavedAt.toISOString()) }}</span
                                >
                            </div>
            ...
            <form class="space-y-6" @submit.prevent>
                <!-- Form Fields -->
                <div
                    v-for="field in assignment.review_evaluation_form.review_form_fields"
                    :key="field.id"
                    class="space-y-4"
                >
                    <Card>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        >
                                            {{ field.label }}
                                            <span v-if="field.is_required" class="text-red-500 ml-1"
                                                >*</span
                                            >
                                        </Label>
                                        <p
                                ...
                                <!-- Radio Group -->
                                <RadioGroup
                                    v-else-if="getFieldComponent(field) === 'radio'"
                                    v-model="form.responses[field.id]"
                                    :disabled="!canEdit"
                                    class="space-y-2"
                                >
                                ...
                                >
                                    ...
                                    >
                                        <Checkbox
                                            :id="`${field.id}_${option.id}`"
                                            :checked="
                                                form.responses[field.id]?.includes(
                ...
                >
                    <div class="flex items-center space-x-4">
                        ...
                        <div
                            v-if="!requiredFieldsCompleted"
                            class="flex items-center space-x-2 text-amber-600"
                        >
                            <AlertTriangle class="h-4 w-4" />
                            <span class="text-sm">Lengkapi isian wajib untuk mengirim</span>
                        </div>
                    </div>
                        ...
                        <Dialog v-if="canEdit" v-model:open="showSubmitDialog">
                            ...
                            <DialogContent>
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Apakah anda yakin ingin mengirim evaluasi ini? Setelah
                                        dikirim, Anda tidak akan dapat mengubah jawaban Anda.
                                    </DialogDescription>
                                </DialogHeader>
