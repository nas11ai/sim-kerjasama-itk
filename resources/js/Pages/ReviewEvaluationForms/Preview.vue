            ...
            <Card>
                <CardContent class="p-6">
                    <div class="grid gap-6 md:grid-cols-2">
                        <div class="space-y-4">
                            <div>
                                <h3 class="text-lg font-semibold">{{ evaluationForm.title }}</h3>
                                <p v-if="evaluationForm.description" class="text-muted-foreground mt-1">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div v-if="getFieldComponent(field) === 'input'">
                                            <Input :type="getInputType(field.field_type.name)"
                                                :placeholder="`Sample ${field.label.toLowerCase()}`" disabled
                                                class="w-full" />
                                        </div>

                                        <!-- Textarea -->
                                        <div v-else-if="getFieldComponent(field) === 'textarea'">
                                            <Textarea :placeholder="`Sample ${field.label.toLowerCase()} response...`"
                                                disabled rows="3" class="w-full" />
                                        </div>
                                            ...
                                            <Select disabled>
                                                ...
                                                <SelectContent>
                                                    <SelectItem v-for="option in field.review_form_field_options"
                                                        :key="option.id" :value="option.value || option.label">
                                                        {{ option.label }}
                                                    </SelectItem>
                                                </SelectContent>
                                            </Select>
                                            ...
                                            <RadioGroup disabled class="space-y-2">
                                                <div v-for="option in field.review_form_field_options" :key="option.id"
                                                    ...
                                                    <RadioGroupItem :value="option.value || option.label"
                                                        :id="`preview_${field.id}_${option.id}`" disabled />
                                                    <Label :for="`preview_${field.id}_${option.id}`" class="opacity-60">
                                                        {{ option.label }}
                                                    </Label>
                                                </div>
                                            </RadioGroup>
                                        ...
                                        <div v-else-if="getFieldComponent(field) === 'checkbox'">
                                            <div class="space-y-2">
                                                <div v-for="option in field.review_form_field_options" :key="option.id"
                                                    class="flex items-center space-x-2">
                                                    <Checkbox :id="`preview_${field.id}_${option.id}`" disabled />
                                                    <Label :for="`preview_${field.id}_${option.id}`" class="opacity-60">
                                                        {{ option.label }}
                                                    </Label>
                                                </div>
