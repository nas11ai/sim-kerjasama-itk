...
const validationIssues = computed(() => {
    ...
    if (props.formData.access_controls.length === 0)
        issues.push('Minimal satu kontrol akses ditambahkan')
    if (!props.formData.phase.phase_type_id) issues.push('Jenis fase wajib diisi')

    if (props.formData.phase.use_existing) {
        if (!props.formData.phase.existing_phase_id)
            issues.push('Tahap yang sudah ada harus dipilih')
    } else {
    ...
    if (props.formData.submission_period.use_existing) {
        if (!props.formData.submission_period.existing_period_id)
            issues.push('Periode yang sudah ada harus dipilih')
    } else {
        if (!props.formData.submission_period.new_period_name)
            issues.push('Judul periode baru wajib diisi')
        if (props.formData.submission_period.dates.length === 0)
            issues.push('Minimal satu tanggal pengajuan diperlukan')
    }
    ...
    <div class="space-y-6">
            ...
            <CardContent class="space-y-3">
                <div class="grid gap-3 md:grid-cols-2">
                    <div>
                        ...
                        <p class="font-medium">
                            {{ formData.form.title || 'Belum diisi' }}
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-muted-foreground">Tipe</p>
                        <Badge variant="outline">
                            {{ getFormTypeName(formData.form.form_type_id) }}
                        </Badge>
                    </div>
                </div>
                ...
                <div>
                    <p class="text-sm text-muted-foreground mb-2">
                        Isian Formulir ({{ formData.form.fields.length }})
                    </p>
                    <div v-if="formData.form.fields.length > 0" class="space-y-2">
                        <div
                            v-for="(field, index) in formData.form.fields"
                            :key="field.temp_id"
        ...
        <Card>
            ...
            <CardContent>
                <div v-if="formData.access_controls.length > 0" class="space-y-2">
                    <div
                        v-for="(control, index) in formData.access_controls"
                        :key="control.temp_id"
                ...
                <p v-else class="text-sm text-muted-foreground">
                    Belum ada kontrol akses yang diatur
                </p>
            </CardContent>
        </Card>
            ...
            <CardContent class="space-y-3">
                <div class="grid gap-3 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <Badge variant="outline">
                            {{ getPhaseTypeName(formData.phase.phase_type_id) }}
                        </Badge>
                    </div>
                </div>
                <div>
                ...
                <div v-if="!formData.phase.use_existing && formData.phase.new_phase_description">
                    <p class="text-sm text-muted-foreground">Deskripsi</p>
                    <p class="text-sm">
                        {{ formData.phase.new_phase_description }}
                    </p>
                </div>
                <div>
        ...
        <Card v-if="formData.phase.needs_review">
            ...
            <CardContent>
                <div v-if="formData.evaluation_forms.length > 0" class="space-y-3">
                    <div
                        v-for="(evalForm, index) in formData.evaluation_forms"
                        :key="evalForm.temp_id"
                    ...
                    >
                        <div class="flex items-center justify-between">
                            ...
                            <Badge
                                :variant="evalForm.is_required ? 'destructive' : 'secondary'"
                                class="text-xs"
                            >
                                {{ evalForm.is_required ? 'Wajib' : 'Opsional' }}
                            </Badge>
                        </div>
                        <p v-if="evalForm.description" class="text-sm text-muted-foreground">
                ...
                <p v-else class="text-sm text-muted-foreground">
                    Belum ada formulir evaluasi yang diatur
                </p>
            </CardContent>
        </Card>
            ...
            <CardContent class="space-y-3">
                ...
                <div>
                    ...
                    <p class="font-medium">
                        {{
                            formData.submission_period.use_existing
                                ? getPeriodName(formData.submission_period.existing_period_id) ||
                                  'Belum dipilih'
        ...
        <Alert>
            ...
            <AlertDescription>
                ...
                <p class="text-sm">
                    Mohon tinjau semua informasi di atas. Setelah dibuat, Anda masih dapat mengedit
                    komponen individual, tetapi lebih mudah untuk melakukannya dengan benar pada
                    percobaan pertama.
                </p>
            </AlertDescription>
        </Alert>
