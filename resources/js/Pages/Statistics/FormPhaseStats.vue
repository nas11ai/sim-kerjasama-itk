...
const periodData = computed(() => {
    return props.formPhaseByPeriod?.map(period => ({
...
const facultyData = computed(() => {
    ...
    const filtered = selectedFaculties.value.length
        ? props.formPhaseFaculty.filter(item =>
            selectedFaculties.value.includes(item.faculty_id)
        )
                            ...
                            <CardContent>
                                <div class="space-y-3">
                                    <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                                        ...
                                        <Badge variant="secondary" class="text-lg">{{ totalStats.pending }}</Badge>
                                    </div>
